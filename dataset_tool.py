...
class TFRecordExporter:
    ...
    def add_image(self, img):
        ...
        for lod, tfr_writer in enumerate(self.tfr_writers):
            ...
            quant = np.rint(img).clip(0, 255).astype(np.float32)
...
def create_from_numpy(tfrecord_dir, image_dir, shuffle):
    print('Loading npy from "%s"' % image_dir)
    image_filenames = sorted(glob.glob(os.path.join(image_dir, '*.npy')))
    if len(image_filenames) == 0:
...
def execute_cmdline(argv):
    ...
    # ----- new added -----
    p = add_command('create_from_numpy', 'Create dataset from a directory full of numpy.',
